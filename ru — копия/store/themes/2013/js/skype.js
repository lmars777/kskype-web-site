			...
			if (ready.length) {
				...
				for (var i = 0, len = ready.length; i < len; i++) {
					...
					(function() {
						...
						// @TODO Detect full URL’s to not use prefix
	...
	return {
		...
		require: function(module, opts, initParams) {
			...
			if (typeof opts == 'object' && opts.src) {
				...
				SKYPE.log('There was no ‘src’ specified for module ‘' + module + '’. Required since version Loader v2.0', 'warn');
				return false;
...
SKYPE.Navigation = function() {

	/*
	 * Toggles navigation states on mobile
	 * Additionally resize navigation area to the content size
	 *
	 * @name		toggleMobileNavigation
	 * @methodOf	SKYPE.Navigation
	 */
	var _toggleMobileNavigation = function() {

			$('#mobileMenu,#mobileMenuLink').on('click', function(event) {

				event.preventDefault();

				$('.mainNavigation').toggleClass('mobileShow');

				//resizing navigation area height to the page content height
				if ($('.mainNavigation').height() < $('body').height()) {

					$('.mainNavigation').height($('body').height());
				}

				$('nav .mainNavigation > ul > li:not(.singleItem) > a:not(#closeMenu, #homeLink)').each(function() {

					var initialText = $(this).html(),
						initialElement = $(this);

					$(this).hide();
					$(this).parent().prepend('<p class="paragraphLink">' + initialText + '</p>');
			...
			$('#closeMenu').on('click', function(event) {
				...
				$('nav .mainNavigation > ul p.paragraphLink').each(function() {
			...
			// Attach responsive state listener to the nav element.
			$(window).on('responsiveStateChange', function(event) {

				if (SKYPE.ResponsiveState.calculateCurrentState() !== 'mobile') {

					$('.mainNavigation').removeAttr('style').removeClass('mobileShow');
		...
		/*
		 * Resizes navigation elements in IE7/IE6
		 *
		 * @name		ieNavigationElementsResizer
		 * @methodOf	SKYPE.Navigation
		 */
		_ieNavigationElementsResizer = function() {

			if ($.browser.msie && $.browser.version < 8) {

				if ($('.mainNavigation').length > 0) {

					$('.mainNavigation .sub li').each(function() {
		...
		/*
		 * Enables attaching CSS styles to nav elements other than 'a' on hover.
		 * Assists only IE6 and lower.
		 *
		 * @name		ieHoverListener
		 * @methodOf	SKYPE.Navigation
		 */
		_ieHoverListener = function() {
				...
				$('.mainNavigation ul li').each(function() {
					...
					$(this).hover(function() {
						...
						if ($(this).hasClass('singleItem')) {
							...
							$(this).toggleClass('singleItemHover');
						}
					});
				});
		...
		/*
		 * Adds username to the navigation and shows the logged in state
		 *
		 * @name		showUserData
		 * @methodOf	SKYPE.Navigation
		 */
		_showUserData = function(username) {

			if (typeof username === 'undefined') return;

			SKYPE.ui.updateContent({
				'skypename': username
			});

			$('nav').removeClass('isNotAuthenticated').addClass('isAuthenticated');
		};

		/*
		 * Shows not logged in state in the navigation
		 *
		 * @name		showUserData
		 * @methodOf	SKYPE.Navigation
		 */
		_hideUserData = function() {
			
			$('nav').addClass('isNotAuthenticated').removeClass('isAuthenticated');
		},

		/*
		* Makes the first level links not clickable if they have children elements
		*/
		_linksNotClickable = function() {

			$('nav .mainNavigation > ul > li:not(.singleItem) > a:not(#closeMenu, #homeLink)').on('click', function() {
		...
		/*
		* Sets the proper meta viewport parameter on tablets
		*/
		_viewportWidthListener = function() {
	...
	return {
		init: function() {
			...
			_ieNavigationElementsResizer();
			_ieHoverListener();
			_linksNotClickable();
			_viewportWidthListener();
		},

		toggleUserState: function(userState, username) {

			if (userState === 'authenticated') {
				_showUserData(username);
			} else if (userState === 'nonAuthenticated') {
				_hideUserData();
			}
...
$(document).ready(function() {
	...
	SKYPE.Navigation.init();
});

}
/*
     FILE ARCHIVED ON 17:11:19 Jan 21, 2013 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 13:25:57 Nov 02, 2024.
...
playback timings (ms):
  ...
  esindex: 0.01
  cdx.remote: 21.233
  LoadShardBlock: 396.754 (3)
  PetaboxLoader3.datanode: 265.396 (6)
  PetaboxLoader3.resolve: 233.22 (2)
  load_resource: 141.821
*/