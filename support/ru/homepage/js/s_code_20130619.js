...
/*global document, event, s_allowLocal, s_allowSurvey, s_impression, window, location, s_referrer_mask, s_pageURL_mask, s_plugin_external*/
/*jshint onevar: true, undef: true, eqeqeq: true, plusplus: true, bitwise: true, regexp: true, strict: false, newcap: true, immed: true, indent: 2, nomen: false*/
...
var s_account = s_account || ((location.hostname.indexOf(".skype.com") !== -1) ? "skypeglobalprod" : "skypeglobaldev");
...
function s_doPlugins(s) {
  ...
  s.campaign = s.decodeMultiple(s.campaign);

  if (typeof s.campaign === "string") {
    if (s.campaign.toUpperCase().indexOf('AF') === 0) {
      s.eVar34 = "D=v0";
    }
  }
  ...
  s.eVar1 = s.decodeMultiple(s.eVar1); // decode if encoded
  ...
  s.visitorID = s.getCookie("skype_s_vi") || "";
  s.prop31 = (s.visitorID && "D=vid") || "";
  s.prop30 = s.prop30 || (((s.getCookie("s_vi") || "").match(/\|([^\[]*)/) || [])[1] || "");
  if (typeof s.linkTrackVars === "string" && s.linkTrackVars !== "") { // on s.tl()
  ...
  // User Type (based on Flash) -v18
  if (s.eVar6 === "") { // no version
    if (s.prop40 === "flash not detected") {
  ...
  /*when hitting a 404 capture the referrer*/
  ...
  // mask url sensitive info
  try {
    s.pageURL = (s.wd.location.href || "").toLowerCase();
    s.pageURL = s.pageURL.replace(/(username=[^&]*|skype-session=[^&]*|nonce=[^&]*)/g, "mask");
    if (typeof s_pageURL_mask !== "undefined") {
      s.pageURL = s.pageURL.replace(s_pageURL_mask, "mask");
    }
    s.referrer = s.referrer || document.referrer;
    s.referrer = s.referrer.replace(/(username=[^&]*|skype-session=[^&]*|nonce=[^&]*)/g, "mask");
    if (typeof s_referrer_mask !== "undefined") {
      s.referrer = s.referrer.replace(s_referrer_mask, "mask");
    }
  } catch (e) {}

  s.eVar40 = s.newPayingUser(s.eVar41);

  s.prop22 = s.eVar26 = s.getNewRepeat(365 * 2);
  s.eVar29 = (s.eVar3 && "D=v3");

  if (typeof s_plugin_external === "function") {
    s_plugin_external();
  }

  /* WEBAN-1373 
   * s.prop23 needs to be previously defined, from: s.prop23=s.getPreviousValue(s.pageName, ...)
   * this code will overwrite any previous value of s.referrer
   */
  if (typeof s.prop23 === "string" && s.prop23 !== "") {
    s.referrer = (((s.prop23.indexOf('/login-form/microsoftID') !== -1) && "https://web.archive.org/web/20160605064428/http://www.skype.com/login-form/microsoftID")  ||
                 ((s.prop23.indexOf('/signup-form/microsoftID') !== -1) && "https://web.archive.org/web/20160605064428/http://www.skype.com/signup-form/microsoftID") ||
                 ((s.prop23.indexOf('/login-form/facebook')     !== -1) && "https://web.archive.org/web/20160605064428/http://www.skype.com/login-form/facebook")     ||
                 ((s.prop23.indexOf('/signup-form/facebook')    !== -1) && "https://web.archive.org/web/20160605064428/http://www.skype.com/signup-form/facebook"));
  }

  s.prop27 = "20130619|H.25.4.";
}
...
/*
 * Custom: s.decodeMultiple
 *   decode code if encoded multiple times
 */
s.decodeMultiple = function (value) {
  var result = value,
      i;
  if (typeof result === "string") {
    for (i = 0; i < 10 && result.indexOf('%') !== -1; i = i + 1) {
      try {
...
/*
 * Custom: s.newPayingUser (WEBAN-1153)
 */
s.newPayingUser = function (registrationDaysAgo) {
  try {
    if ((typeof registrationDaysAgo === "string" &&
                registrationDaysAgo !== "") ||
         typeof registrationDaysAgo === "number") {

      var reg_days_ago = "",
                   npu = "",
                   date_now = new Date(),
                   date_now_utc = new Date(date_now.getUTCFullYear(),
                                           date_now.getUTCMonth(),
                                           date_now.getUTCDate(),
                                           date_now.getUTCHours(),
                                           date_now.getUTCMinutes(),
                                           date_now.getUTCSeconds());

      reg_days_ago = parseInt(registrationDaysAgo, 10);

      // on a purchase
      if (typeof s.events !== "undefined"     && // s.events is defined
          s.events.indexOf('purchase') !== -1 && // s.events includes a purchase
          reg_days_ago <= 31                  && // registration less than month
          typeof s.prop3 !== "undefined"      && // prop3 is defined
          s.prop3 !== "") {                      // prop3 is not empty
        s.events = s.events + ",event41:" + s.prop3;
      }

      if (reg_days_ago === 0) { // same day
        npu = "in day of registration";
      } else {
        if (date_now_utc.getDay() - reg_days_ago >= 0) { // this week
          npu = "in week of registration";
        } else {
          if (date_now_utc.getDate() - reg_days_ago >= 0) { // this month
            npu = "in month of registration";
          } else {
            npu = "registration in previous month";
          }
        }
      }
      return npu;
...
s.trackPage = function (_t) {
  ...
  // impressions merge
  if (typeof s_impression !== "undefined" &&
      s_impression.length !== 0) {
    args = s.mergeImpressionData(s_impression, args);
    window.s_impression = []; // clean
  }
...
/* sets s.linkTrackVars and linkTrackEvents automatically*/
s.tl2 = function (args, el) {
  ...
  for (key in args) {
    ...
    if (key.indexOf('events') !== -1) {
      // also cleans serialization when present
      s.linkTrackEvents = s.linkTrackEvents.replace("None", args[key].split(":")[0]);
    }
  }

  // when element clicked is a valid link
...
/*
 * Custom: merge impression data
 */
s.mergeImpressionData = function (impression_arr, _t) {
  var _i, _var,
      _impressions = impression_arr || [],
      _data = _t || {};
  ...
  for (_i in _impressions) {
    for (_var in _impressions[_i]) {
      ...
      } else { // _t data takes priority
        _data[_var] = _data[_var] || _impressions[_i][_var];
      }
    }
  }
...
if (typeof s_module_external === "function") {
//  s_module_external();
}
...
/*
     FILE ARCHIVED ON 06:44:28 Jun 05, 2016 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 11:03:05 Oct 31, 2024.
...
playback timings (ms):
  ...
  esindex: 0.006
  cdx.remote: 7.829
  LoadShardBlock: 216.409 (3)
  PetaboxLoader3.datanode: 93.792 (4)
  PetaboxLoader3.resolve: 181.029 (2)
  load_resource: 92.5
*/